...
class AspectDataset(Dataset):
    def __init__(self, path, tokenizer: CLIPTokenizer, batch_size = 1, mask = False, controll = False, prompt = ""):
    ...
    def __getitem__(self, i):
        ...
        if self.controll:
            controlls = torch.stack([torch.tensor(np.load(os.path.join(self.path, sample + ".npz"))["controll"]).unsqueeze(0) for sample in samples])
            controlls.to(memory_format=torch.contiguous_format).float()
            captions = [self.prompt for _ in range(len(samples))]
        else:
            controlls = None
            
        return {"latents":latents,"caption":captions,"mask": masks,"controll": controlls}
